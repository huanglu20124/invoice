...
public class InvoiceServiceImpl implements InvoiceService {
	...
	public void addRecognizeInvoice(Map<String, Object>ans_map, Integer user_id,
		...
		if(user == null || user.getAction_auth() < 2 || user.getInvoice_auth() < 2){
			ans_map.put(Const.ERR, "权限不足");
		}else {
	...
	public void addOrUpdateInvoiceModel(Map<String, Object>ans_map, Integer user_id,
		...
		//首先进行权限判断
		User user = userDao.getUserById(user_id);
		if(user == null || user.getAction_auth() < 2 || user.getModel_auth() < 2){
			ans_map.put(Const.ERR, "权限不足");
		}else {
			// 添加新的发票模型
				...
				if (message != null) {
					if (message.getMsg_id() == 1) {
						...
						if(customerSocket != null){
							MessageUtil.sendMessage(customerSocket.getOutputStream(),
						    		message.getMsg_id(), message.getJson_str(),null);
	...
	public void getAllModel(Map<String, Object> ans_map, Integer user_id, Integer start) {
		...
		//首先进行权限判断
		User user = userDao.getUserById(user_id);
		if(user == null || user.getModel_auth() < 1){
			ans_map.put(Const.ERR, "权限不足");
		}else {
	...
	public void deleteAllModel(Map<String, Object> ans_map, Integer user_id, Integer thread_msg) {
		...
		//首先进行权限判断
		User user = userDao.getUserById(user_id);
		if(user.getAction_auth() < 2 || user.getModel_auth() < 2){
			ans_map.put(Const.ERR, "权限不足");
		}else {
			// 1生成一条行为，插入action表，并获取返回的action_id
			Integer action_id = invoiceDao.addNewModelAction(user_id);
	...
	public void UpdateRecognizeSpeed(Map<String, Object> ans_map, Integer user_id,Integer delay,ServletContext servletContext) {
		//首先进行权限判断
		User user = userDao.getUserById(user_id);
		if(user == null || user.getAction_auth() < 2){
			ans_map.put(Const.ERR, "权限不足");
		}else {
